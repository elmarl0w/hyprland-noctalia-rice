...
### Обновление обоев и палитры

Экран входа получает картинку и цвета через синхронизацию из оболочки:

```bash
noctalia msg greeter-sync
```

Команда стажирует файлы в `/run/user/<uid>/noctalia-greeter-sync/`, затем
`noctalia-greeter-apply-appearance` раскатывает их в `/var/lib/noctalia-greeter/`.
Второй шаг поднимается до root, поэтому будет **один** запрос пароля.

Если команда отвечает `unknown command` — гритер ещё не установлен и не настроен.
Она начинает работать только после того, как пакет на месте, а `greetd` настроен.

**Не прописывай обои декларативно в `greeter.toml`.** Ключи `[appearance.wallpaper]`
там имеют приоритет над синхронизацией и не перетираются ею — то есть намертво
фиксируют картинку, и `greeter-sync` перестаёт что-либо менять. Оставь блок
внешнего вида пустым и обновляй синхронизацией.
...
Держи `auto_sync = false` и обновляй экран входа вручную командой
`noctalia msg greeter-sync`, когда это действительно нужно — один запрос пароля
по требованию вместо запроса при каждой смене обоев.
...
**Обои экрана входа замерли.** Если в `greeter.toml` прописан
